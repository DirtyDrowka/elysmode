...
// Основной промпт. Recraft режет агрессивную сексуальную лексику —
// если триггернёт content filter, retry'имся на безопасный fallback
// ниже (buildCharacterPromptSafe).
function buildCharacterPrompt(appearance: string): string {
  return (
    `деталезированный клешированный аниме стиль, максимум деталей, бликов, шума\n` +
    `Вызывающий сексуализированный ярий образ\n` +
